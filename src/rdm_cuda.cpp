...
RDM_cuda::RDM_cuda(Settings_swe* settings_swe, RDM* rho, cudaStream_t* stream){
    _rho_cpu = rho;
...
void RDM_cuda::_allocate(){
    int nr1 = _settings_swe->nr1;
    int nr2 = _settings_swe->nr2;
    int nr3 = _settings_swe->nr3;
    int num_orbitals = _settings_swe->num_orb;
...
void RDM_cuda::_create_cufft_plan(){
    ...
    cufftPlanMany(&_cufft_plan, 2, dims, 
                  embed, _settings_swe->num_orb*_settings_swe->num_orb, 1,
                  embed, _settings_swe->num_orb*_settings_swe->num_orb, 1,
                  CUFFT_Z2Z, _settings_swe->num_orb*_settings_swe->num_orb);
    cufftSetStream(_cufft_plan, *_stream);
}

void RDM_cuda::copy_cpu_to_gpu(){
    int num_points = _settings_swe->nr1*_settings_swe->nr2*_settings_swe->nr3;
    int num_orbitals = _settings_swe->num_orb;
...
void RDM_cuda::convert_to_k(){
    ...
    int num_points = _settings_swe->nr1*_settings_swe->nr2;
    int num_orbitals = _settings_swe->num_orb;
